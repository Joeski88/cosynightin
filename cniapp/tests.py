...
class MovieSearchViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Movies.objects.create(id=1, movie_title="Inception",
                              genres="Sci-Fi",
                              original_release_date="2010-07-16")
        Movies.objects.create(id=2, movie_title="Interstellar",
                              genres="Sci-Fi",
                              original_release_date="2014-11-07")
        Movies.objects.create(id=3, movie_title="The Dark Knight",
                              genres="Action",
...
class LeaveReviewViewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.movie = Movies.objects.create(id=1, movie_title="Inception",
                                          genres="Sci-Fi",
                                          original_release_date="2010-07-16")

    def test_leave_review_authenticated(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post(reverse('leave_review',
                                    args=[self.movie.id]), {
            'title': 'Great movie!', 'slug': 'great-movie',
            'content': 'Great movie!', 'status': 1,
            'rating': 5
        })
    ...
    def test_leave_review_unauthenticated(self):
        ...
        # Redirect to login page (unauthenticated access)
        self.assertEqual(response.status_code, 302)
...
class home_page_viewTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        movie = Movies.objects.create(id=1, movie_title="Inception",
                                      genres="Sci-Fi",
                                      original_release_date="2010-07-16")
        Review.objects.create(movie=movie,
                              author=User.objects.create_user('user1_unique'),
                              title="Amazing movie!", slug="amazing-movie",
                              content="Amazing movie!", status=1)
        Review.objects.create(movie=movie,
                              author=User.objects.create_user('user2_unique'),
                              title="Loved it!", slug="loved-it",
                              content="Loved it!", status=1)
